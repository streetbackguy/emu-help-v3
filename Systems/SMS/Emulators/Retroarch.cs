    ...
    public override bool FindRAM(ProcessMemory _process)
    {
        ...
        RamBase = _process.Is64Bit
            ? core.ModuleName switch
            {
                ...
                "smsplus_libretro.dll" => _process.Scan(new MemoryScanPattern(3, "48 8B 05 ?? ?? ?? ?? 48 83 C0") { OnFound = addr => _process.DerefOffsets(addr + 0x4 + _process.Read<int>(addr), _process.Read<byte>(addr + 0x7)) }, baseScanAddr, 0x100),
                "gearsystem_libretro.dll" => _process.Scan(new MemoryScanPattern(3, "48 8B 0D") { OnFound = addr => _process.DerefOffsets(addr + 0x4 + _process.Read<int>(addr), 0x0, 0x18, 0xC000) }, baseScanAddr, 0x100),
                _ => IntPtr.Zero,
            }
            : core.ModuleName switch
            {
                ...
                "smsplus_libretro.dll" => _process.Scan(new MemoryScanPattern(1, "B8 ?? ?? ?? ?? B9 00 00 00 00") { OnFound = _process.ReadPointer }, baseScanAddr, 0x100),
                "gearsystem_libretro.dll" => _process.Scan(new MemoryScanPattern(2, "8B 0D ?? ?? ?? ?? E8") { OnFound = addr => _process.DerefOffsets(addr, 0x0, 0x0, 0xC, 0xC000) }, baseScanAddr, 0x100),
                _ => IntPtr.Zero,
            };
